    ...
    if user:
        try:
            ...
            try:
                ...
                app.logger.error(f"Reset email send failed: {str(e)}")
...
def update_profile():
    ...
    if 'username' in data:
        ...
        if not re.match(r'^[A-Za-z][A-Za-z0-9._]*$', new_username):
            return jsonify({'error': 'Username must start with a letter and contain only letters, numbers, dots, and underscores'}), 400
...
def test():
    ...
    if not current_app.config.get('DEBUG'):
        return jsonify({'error': 'Not found'}), 404
    return jsonify({'status': 'Auth blueprint is working!'}), 200
